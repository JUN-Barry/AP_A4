...
public class UpdateProfileController {
 ...
 // pass the primary stage to the controller
    ...
    private void confirmClick(ActionEvent event) {
        // setText here, retrieve the data fromDB, based on USERNAME, IF SAME in the DB, output error message
        ...
        if (SignUPModel.CheckUsername(Newusername)) {
			// we have delete the user info before, so insert the value into database directely, no duplication
			SignUPModel.SignUpUserInfoINTOTable(Newusername, Newpassword, NewfirstName, NewlastName);
			statusLabel.setText("User information has been updated.");


		} else {
			statusLabel.setText("Username has been registed. Change one.");
		}
